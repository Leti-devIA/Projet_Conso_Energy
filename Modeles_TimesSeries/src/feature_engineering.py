...
def feature_engineering_pipeline(df, config_path="config/config.yaml"):
    ...
    if target_col not in df.columns:
        print(f"⚠️  Colonne '{target_col}' non trouvée dans le DataFrame")
        print(f"📋 Colonnes disponibles : {list(df.columns)}")
        raise ValueError(f"Colonne cible '{target_col}' manquante. Vérifiez que le preprocessing a été exécuté.")
