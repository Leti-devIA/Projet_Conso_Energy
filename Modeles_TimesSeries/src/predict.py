...
def load_model_artifacts(model_dir="models/saved", use_latest=True):
    """
...
def predict_future(historique_df, meteo_future_df, model_dir="models/saved", 
                   config_path="config/config.yaml", horizon=None):
    """
    ...
    model, scaler_X, scaler_y, model_config = load_model_artifacts(model_dir)
...
if __name__ == "__main__":
    ...
    print("EXEMPLE : Prédiction sur 15 jours (360 heures)")
    print()
    
    # Charger l'historique (48 dernières heures minimum)
    # historique = pd.read_csv("data/processed/historique_48h.csv")
    
    # Charger les données météo futures
    # meteo_future = pd.read_csv("data/raw/meteo_previsions_15jours.csv")
