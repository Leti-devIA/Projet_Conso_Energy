...
from app.repository.fabric_repository import FabricRepository, FabricDmlNotSupportedError
...
# Chemins
# ============================================================
# Remonte 4 niveaux : models.py → routers/ → app/ → api-inference/ → api/ → Modeles_TimesSeries/
MODELS_DIR = Path(__file__).resolve().parents[4] / "models" / "saved"
FABRIC_OUTBOX_PATH = Path(__file__).resolve().parents[4] / "exports" / "fabric_outbox.jsonl"
...
async def get_active_model(prm: str) -> ModelRegistryResponse:
    ...
    try:
        ...
        try:
            ...
            # Fabric non configuré, essaie local
            logger.warning("⚠️  Fabric non configuré, utilise fichier local")
            pass
