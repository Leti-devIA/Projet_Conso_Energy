...
def load_achats() -> pd.DataFrame:
    ...
    for num_col in ["VOLUME_TOTAL_PERIODE", "COUT_TOTAL_PERIODE", "PRIX_MOYEN", "PRIX_FIXATION", "FIXATION_PUISSANCE_ACHAT_MW"]:
...
def _resolve_selected_prm(site_label: str) -> str | None:
    """Résout le PRM du site sélectionné depuis les jeux filtrés puis globaux."""
    for _src in (filtered_pred, preds_df, filtered_hist, hist_df):
        if _src.empty or "site_label" not in _src.columns or "prm" not in _src.columns:
            continue
        _mask = _src["site_label"] == site_label
        if _mask.any():
            return str(_src.loc[_mask, "prm"].astype(str).iloc[0])
    return None


chosen_prm        = _resolve_selected_prm(selected_sites[0]) if not multi_site else None
...
if multi_site:
    ...
    with _pc2:
        st.plotly_chart(fig_pie(filtered), width='stretch')
    st.plotly_chart(fig_consumption_curve(filtered, aggregate=True), width='stretch')
    st.plotly_chart(fig_annual_total_bar(all_years_total), width='stretch')
    st.plotly_chart(fig_yearly_bar(all_years_total), width='stretch')
else:
    st.plotly_chart(fig_consumption_curve(filtered, aggregate=False), width='stretch')
    st.plotly_chart(fig_annual_total_bar(all_years_total), width='stretch')
...
if not multi_site:
    ...
    # ── Métriques actuelles ────────────────────────────────────────────────
    _current_metrics = load_mlflow_metrics(chosen_prm) if chosen_prm else None
    ...
    if _current_metrics:
        ...
        st.markdown(
            "<div class='info-subtle'>PRM introuvable pour ce site : la section réentraînement est indisponible.</div>",
            unsafe_allow_html=True,
        )
    ...
    if current_role == "admin" and chosen_prm:
        _already_trained = (
        ...
        with st.expander("Réentraîner le modèle", expanded=False):
            ...
            if (
                ...
                with _accept_col:
                    if st.button("✅ Enregistrer le nouveau modèle", width='stretch', type="primary"):
                        ...
                        st.rerun()
                with _reject_col:
                    if st.button("Conserver le modèle actuel", width='stretch'):
                        st.session_state.training_new_metrics = None
...
if multi_site:
    ...
    if prices_df.empty:
        ...
        if not monthly_prices.empty:
            st.plotly_chart(fig_price_curve(monthly_prices), width='stretch')
        else:
